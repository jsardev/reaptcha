...
class Reaptcha extends Component<Props> {
  ...
  _getRecaptchaScripts(): Array<any> {
    return Array.from(document.scripts).filter(
      script => script.src.indexOf('recaptcha') > 0
    );
  }

  UNSAFE_componentWillMount(): void {
    const isRecaptchaScriptPresent = this._getRecaptchaScripts().length > 0;

    if (this.props.inject && !isRecaptchaScriptPresent) {
  ...
  componentWillUnmount(): void {
    ...
    if (this.injected) {
      this._getRecaptchaScripts().forEach(script => script.remove());
    }
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      if (this._isAvailable() && this.container) {
        ...
        this.id = window.grecaptcha.render(this.container, {
          ...
          hl: isInvisible ? null : this.props.hl
        });
